...
[NodeCategory("문자열")]
[NodeDescription("문자열이 지정된 부분 문자열로 시작하는지 확인합니다.")]
public class StringStartsWithNode : NodeBase
{
    [NodeInput("입력 문자열")]
    public InputPort<string> Input { get; set; }
    
    [NodeProperty("시작 문자열", CanConnectToPort = true)]
    ...
    public StringStartsWithNode(INodeCanvas canvas, Guid guid) : base(canvas, guid) {
    ...
    )
    {
        ...
        // value가 비어있지 않은 경우에만 StartsWith 수행
        if (!string.IsNullOrEmpty(value))
        {
            if (IgnoreCase.Value)
            {
                ...
                result = input.StartsWith(value, comparison);
            }
            else
            {
                // 기본 StartsWith 호출 (대소문자 구분)
                result = input.StartsWith(value);
            }
        }
        else
        {
            // 시작 문자열이 비어있으면 모든 문자열이 빈 문자열로 시작한다고 간주
            result = true;
        }
